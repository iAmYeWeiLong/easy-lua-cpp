...
static void deleter(uv_prepare_t * handle) {
	//uv_close里面包含stop动作
...
static int __new__(lua_State* L) {
	...
	auto size = sizeof(std::shared_ptr<uv_prepare_t>**);
	auto self = (std::shared_ptr<uv_prepare_t>**)lua_newuserdata(L, size);//[-0, +1, m]
	*self = new std::shared_ptr<uv_prepare_t>(new uv_prepare_t, deleter);
...
static int __gc(lua_State* L) {
	auto const THIS_CLASS = lua_upvalueindex(1), SELF = 1;//[-0, +0, -]

	auto self = (std::shared_ptr<uv_prepare_t>**)lua_touserdata(L, SELF);
	luaL_argcheck(L, self, SELF, "invalid prepare");
...
static int __init__(lua_State* L) {
	...
	auto const SELF = 1, LOOP_INSTANCE = 2;
	...
	auto ret = uv_prepare_init(*loop, &***self);
	if (ret != 0) {
		luaL_error(L, "cPrepare的__init__失败");
	}
	return 0;
}
...
static int start(lua_State* L) {
	...
	//防止脚本重复地start
	//auto stopRet = uv_prepare_stop(*prepare);// 没有必要真的stop,更换掉脚本的回调函数就可以了
	...
	auto ret = uv_prepare_start(&***self, uv_prepare_cb(common_handle_cb));
...
static const struct luaL_Reg prepare_m[] = {
	{ "__new__", __new__ },
	{ "__gc", __gc },
	{ "__init__", __init__ },
	{ "start", start },
...
int create_prepare_class(lua_State *L) {
	...
	//printf("create_prepare_class lua_gettop==%d ...\n", lua_gettop(L));
	...
	luaL_setfuncs(L, prepare_m, 2);//把类设为全部成员函数的upvalue.[-nup, +0, e]
	return 1;
}
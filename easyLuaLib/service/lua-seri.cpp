...
seri(lua_State *L, struct block *b, int len) {
	...
	auto pBuffer = new std::shared_ptr<uint8_t>(new uint8_t[len], std::default_delete<uint8_t[]>());//
	//auto pBuffer = new std::shared_ptr<uint8_t>(new uint8_t[len], [](uint8_t* p) {delete[] p; printf("fuck_lambda delete \n"); });//
...
luaseri_unpack(lua_State *L) {
	...
	if (lua_type(L,1) == LUA_TSTRING) {
		...
		auto pBuffer = (std::shared_ptr<uint8_t> *)lua_touserdata(L,1);
		buffer = &(**pBuffer);
